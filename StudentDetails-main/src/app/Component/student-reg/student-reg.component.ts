...
export class StudentRegComponent {
  ...
  zoomLevel: number = 1; // Zoom level
  rotation: number = 0; // Rotation angle
  maxZoomLevel: number = 3; // Max zoom level
...
previewDetails() {
  ...
  // On Rotate Left
  rotateLeft() {
    this.rotation -= 90; // Rotate left by 90 degrees
  }

  // On Rotate Right
  rotateRight() {
    this.rotation += 90; // Rotate right by 90 degrees
  }

  // Close Image Modal
  closeImageModal() {
    this.selectedImage = ''; // Clear selected image
